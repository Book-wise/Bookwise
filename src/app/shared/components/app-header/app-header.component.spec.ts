...
/**
 * Thin delegation spec: the header no longer reloads the reference store
 * itself — it hands the switch to the coordinator and keeps its own toasts.
 */
describe('AppHeaderComponent — tenant switch delegation', () => {
  ...
  beforeEach(async () => {
    ...
    component = TestBed.createComponent(AppHeaderComponent).componentInstance;
  });
  ...
  it('delegates to the coordinator and keeps the success toast', () => {
    ...
    expect(config.detail).toContain('Kinesilk Norte');
  });

  it('maps a switch error to the tenant error key with a global error toast', () => {
    tenantSwitch.switchTenant.mockReturnValue(throwError(() => ({ status: 409 })));
    ...
    expect(config.detail).toBe(component.lang.t('auth.switch_tenant_not_ready'));
  });
});
